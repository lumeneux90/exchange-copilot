...
const AVAILABLE_RANGES: CandleRange[] = [
  "day",
  "week",
  "month",
  "year",
...
export function ChartAreaInteractive({ stocks }: { stocks: Stock[] }) {
  ...
  const [selectedTicker, setSelectedTicker] = React.useState(
    () => requestedTicker || stocks[0]?.ticker || ""
  );
  ...
  const lastAppliedTicker = React.useRef("");
  ...
  React.useEffect(() => {
    ...
    if (requestedStock) {
      if (requestedStock.ticker !== lastAppliedTicker.current) {
        lastAppliedTicker.current = requestedStock.ticker;
        ...
        <div className="flex min-w-0 items-center gap-3 self-center">
          ...
          <div className="min-w-0">
            ...
            <div className="text-muted-foreground mt-1 truncate text-sm">
              {selectedStock?.ticker ?? "Выберите инструмент"}
            </div>
          </div>
        </div>
