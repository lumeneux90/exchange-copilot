...
const transactionCurrencyFormatters: Record<string, Intl.NumberFormat> = {
  RUB: rubFormatter,
...
function getMoneyFormatter(currencyCode?: string | null) {
  return transactionCurrencyFormatters[currencyCode ?? "RUB"] ?? rubFormatter;
}

function getCashMovementFormatter(item: PortfolioHistoryItem) {
  if (item.type === "DEPOSIT" || item.type === "WITHDRAWAL") {
    return getMoneyFormatter(item.currencyCode);
  }
            ...
            {items.map((item) => {
              ...
              const amountFormatter =
                item.type === "DEPOSIT" || item.type === "WITHDRAWAL"
                  ? getMoneyFormatter(item.currencyCode)
                  : rubFormatter;
              const cashMovementFormatter = getCashMovementFormatter(item);
              ...
              return (
                <TableRow key={item.id}>
                  ...
                  >
                    {formatSignedCurrency(cashMovement, cashMovementFormatter)}
                  </TableCell>
                </TableRow>
              );
            })}
